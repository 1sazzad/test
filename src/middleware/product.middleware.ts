...
class ProductMiddleware {
	...
	validateProductEdit = (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			if (validationResult.error) {
				console.log(
					"Error occures while validating product edit: ".red,
					validationResult.error.message,
				);
				return responseSender(res, 400, validationResult.error.message);
			}
	...
	validateProductDeletion = (
		...
		try {
			...
			if (validationResult.error) {
				console.log(
					"Error occures while validating product deletion: ".red,
					validationResult.error.message,
				);
				return responseSender(res, 400, validationResult.error.message);
			}
	...
	validateProductFetchById = (
		...
		try {
			...
			if (validationResult.error) {
				console.log(
					"Error occures while validating product fetcb by id: ".red,
					validationResult.error.message,
				);
				return responseSender(res, 400, validationResult.error.message);
			}
	...
	validateProductStatusChange = (
		...
		try {
			...
			if (validationResult.error) {
				console.log(
					"Error occures while validating product status change operation: "
						.red,
					validationResult.error.message,
				);
				return responseSender(res, 400, validationResult.error.message);
	...
	validateRandomProductsFetching = (
		...
		try {
			...
			if (validationResult.error) {
				console.log(
					"Error occures while validating random product fetching: "
						.red,
					validationResult.error.message,
				);
				return responseSender(res, 400, validationResult.error.message);
			...
			console.log(
				"Error occures while validating random product fetching: ".red,
				err.message,
			);
			next(err);
		}
