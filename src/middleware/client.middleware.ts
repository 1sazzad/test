...
class ClientMiddleware {
	...
	validateClientCreation = (
		...
		try {
			...
			if (validationResult.error) {
				console.log(
					"Error occures while validating client creation: ".red,
	...
	validateClientEdit = (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			if (validationResult.error) {
				console.log(
					"Error occures while validating client edit: ".red,
					validationResult.error.message,
				);
				return responseSender(res, 400, validationResult.error.message);
			}
	...
	validateFilteringQueries = (
		...
		try {
			...
			if (validationResult.error) {
				console.log(
					"Error occures while validating client filter queries: "
						.red,
					validationResult.error.message,
				);
				return responseSender(res, 400, validationResult.error.message);
			...
			console.log(
				"Error occures while validating client filter queries: ".red,
				err.message,
			);
			next(err);
		}
